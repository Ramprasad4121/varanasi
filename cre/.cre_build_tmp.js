...
function scorePoolRisk(input, confidential) {
  const factors = [];
  const w = { ...DEFAULT_WEIGHTS, ...confidential.strategyWeights };
  let score = 0;
  if (input.operator && confidential.operatorAllowlist.length > 0) {
    const listed = confidential.operatorAllowlist.some((a) => a.toLowerCase() === input.operator.toLowerCase());
    if (!listed) {
      factors.push({
      ...
      return finish(input.poolId, 1e4, factors, confidential.thresholdBps);
    }
...
function finish(poolId, score, factors, thresholdBps) {
  const riskScoreBps = Math.min(1e4, Math.round(score));
  const decision = riskScoreBps <= thresholdBps ? "ACT" : "SKIP";
  return { poolId, riskScoreBps, decision, factors };
}
var configSchema = exports_external.object({
  aegisApiUrl: exports_external.string().min(1),
  thresholdSecretId: exports_external.string().default("RISK_THRESHOLD_BPS"),
  weightsSecretId: exports_external.string().default("STRATEGY_WEIGHTS"),
  allowlistSecretId: exports_external.string().default("OPERATOR_ALLOWLIST"),
  apiKeySecretId: exports_external.string().default("AEGIS_API_KEY")
});
var publicInputSchema = exports_external.object({
  poolId: exports_external.string().min(1),
  tvlUsd: exports_external.number().nonnegative(),
  volume24hUsd: exports_external.number().nonnegative(),
  alphaScore: exports_external.number().min(-1).max(1).default(0),
  operator: exports_external.string().optional()
});
var parseConfidential = (args) => {
  const thresholdBps = Number(args.thresholdRaw);
  if (!Number.isFinite(thresholdBps) || thresholdBps < 0 || thresholdBps > 1e4) {
    throw new Error(`invalid RISK_THRESHOLD_BPS secret: expected 0..10000, got ${typeof args.thresholdRaw}`);
  }
  let strategyWeights;
  try {
    const parsed = JSON.parse(args.weightsRaw);
    strategyWeights = {
    ...
    throw new Error(`invalid STRATEGY_WEIGHTS secret: expected JSON {liquidity,activity,alpha}: ${err.message}`);
  }
  let operatorAllowlist;
  try {
    const parsed = JSON.parse(args.allowlistRaw);
    ...
    throw new Error(`invalid OPERATOR_ALLOWLIST secret: expected JSON string[]: ${err.message}`);
  }
...
var onHttpTrigger = (runtime2, payload) => {
  ...
  const publicInputs = publicInputSchema.parse(JSON.parse(new TextDecoder().decode(payload.input)));
  const thresholdRaw = runtime2.getSecret({ id: config.thresholdSecretId }).result().value;
  const weightsRaw = runtime2.getSecret({ id: config.weightsSecretId }).result().value;
  const allowlistRaw = runtime2.getSecret({ id: config.allowlistSecretId }).result().value;
  const apiKey = runtime2.getSecret({ id: config.apiKeySecretId }).result().value;
  ...
  let posted = false;
  try {
    const postResponse = new cre.capabilities.HTTPClient().sendRequest(runtime2, {
  ...
  runtime2.log(`Enclave verdict complete. pool=${verdict.poolId} decision=${verdict.decision} score=${verdict.riskScoreBps}bps posted=${posted}`);
  const donRuntime = runtime2.usingTheDons();
  const encodedPayload = encodeAbiParameters(parseAbiParameters("string poolId, uint256 riskScoreBps, string decision"), [verdict.poolId, BigInt(verdict.riskScoreBps), verdict.decision]);
  donRuntime.report({
