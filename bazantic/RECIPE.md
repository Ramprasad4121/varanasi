# AEGIS × Bazantic — "Risk-gated swap check" recipe
...
- `openapi-signal.yaml` — OpenAPI 3.0 spec of the AEGIS x402 signal service, for Bazantic gateway import.
...
2. **Deploy the AEGIS signal service** (or expose it) so it has a public URL:
   ...
   Note the URL: `AEGIS service URL: <YOUR_SERVICE_URL>`.
...
5. **Create the Recipe:** Recipes → New → paste `bazantic/recipe.json`
   ...
   ② AEGIS x402 signal → ③ RiskGuard authorize read → ACT/SKIP).
...
| 2 | 0:50–1:30 | Step ② run: `POST /v1/signal` → 402 → paid 200 via gateway; show signal + confidence + receipt | "Second, the paid AEGIS alpha signal over x402 on Hedera testnet — one cent, settled on-chain." |
...
### Best Recipe ($1k)
- [ ] Recipe uses **≥2 services** in one working flow: The Graph Uniswap data (sponsor API) + AEGIS x402 signal (our API).
...
### Agentify ($1k)
- [ ] AEGIS service imported into Bazantic as an **x402 gateway** (`openapi-signal.yaml`).
