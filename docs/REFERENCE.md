...
| `Akshaya` | *(not yet deployed)* | Deploy via `contracts/script/DeployInventions.s.sol`, then record here + `AKSHAYA_ADDRESS` / `NEXT_PUBLIC_AKSHAYA`.
| `GhatStream` | *(not yet deployed)* | Same script (`GHAT_NO_GATE=1` → zero admin gate); then `NEXT_PUBLIC_GHAT_STREAM`. || `MockERC20` (vUSD) | `0x6169A84cD7430042fb697c2cC131F663212E8b30` | Demo stable token (simulated). |
...
### Secrets files
Live secrets in gitignored `.env` / `.env.local` (chmod 600) **only**.
The old `.env.enc`-in-git pattern is retired — encrypted blobs are treated as
secret-hygiene violations and removed on sight; gitleaks enforces in CI. Keys
never appear as CLI flags (`--key-stdin` or env).
...
|---|---|---|
| Contracts (Foundry) | `forge test` | **143** test functions / 10 suites — offline (mock ENS): rail 89 (TaskEscrow 42, Aegis 29, Akshaya 8, GhatStream 10) + finance 54 |
| Agent (vitest) | `npm test` (`vitest run`) | **165** tests / 18 files — mocked viem/fetch, no live wallet |
| Service (node:test) | `npm test` (`tsx --test`) | **5** tests |
| Service (adversarial HTTP) | `node --test test/adversarial_service.test.mjs` | **1** harness (many probes, built dist/) |

Community finance: 54 forge (SavingsVault 9, ChitPool 7, LoanAgreement 10,
CollateralVault 10, FinancialReputation 9, Gold 9) + 9 agent finance + 5
service finance = 68 finance-aware tests. All finance contracts compile on the
zero-dep `src/lib/` rail (Ownable/Pausable/ERC20/EnumerableSet vendored —
no OpenZeppelin dep).
...
| Check | Job | Runs |
|---|---|---|
| contracts (forge build + test) | forge build + forge test | build + 143 test functions |
| agent (typecheck + tests) | tsc + vitest | 165 tests |
| service (typecheck + build + adversarial) | tsc + build + node --test | finance suite + adversarial harness |
