...
export function execute(
  ...
  switch (agent.id) {
    ...
    case "analyst": {
      ...
      const known = POOLS.find(
        (p) => p.id.toLowerCase() === pool.toLowerCase() || p.name.toUpperCase().includes(pool.toUpperCase()),
      );
      if (!known) {
        return { barPassed: false, output: { pool, error: "unknown pool", known: POOLS.map((p) => p.id) } };
      }
      return {
        barPassed: true,
        output: { pool: known.id, name: known.name, verdict: { decision: "ACT", riskScoreBps: 1800 } },
      };
    }
    case "freelancer": {
    ...
    case "router": {
      ...
      if (!quote) {
        return { barPassed: false, output: { pair, error: "unknown pair", broadcast: false, known: Object.keys(QUOTE_BOOK) } };
      }
      return {
        barPassed: true,
        output: { pair: quote.pair, path: quote.path, amountOut: quote.amountOut, venue: quote.venue, broadcast: false },
      };
    }
    case "keeper": {
    ...
    case "reporter": {
      ...
      if (!hit) {
        return { barPassed: false, output: { topic, error: "unknown topic", citations: [] } };
      }
      return {
        barPassed: true,
    ...
    case "trader": {
      ...
      if (!parsed || price == null) {
        return { barPassed: false, output: { order, error: "unknown order", broadcast: false } };
      }
      return {
        barPassed: true,
        output: { order, quote: { size: parsed.size, price, pair: parsed.pair }, broadcast: false },
      };
    }
    case "dispatcher": {
