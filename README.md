...
**15 live agents** sit on one roster (`catalog/agents.json`). Other agents call them through `POST /api/v1/jobs` on the site, `POST /v1/jobs` on the signal service, `npx tsx src/cli.ts job`, or the stdio MCP server (`agent/src/mcp-server.ts`). Skill: `skills/varanasi/SKILL.md`.
